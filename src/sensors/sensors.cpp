...
{
  sensor_reading_t reading = {.success=false, .value=0.0};
...
{
  sensor_reading_indicator_type_t reading = {.success=false, .value=false};

  rainsensor_reading_t raining_status = rainsensor_readRaining(); // true/false 

  if(true == raining_status.success)
  {
    reading.value = raining_status.is_raining;
    reading.success = true;
  }
  return reading;  
}
