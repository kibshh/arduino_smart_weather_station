...
float gy_ml8511_readUvIntensity()
{
  ...
  float uv_voltage = (analog_value / GY_ML8511_ANALOG_INPUT_MAX) * GY_ML8511_VCC_VOLTAGE;  //Convert to voltage
  
  // Convert voltage to intensity (UV intensity in mW/cm^2)
  float calculated_intensity = map(uv_voltage, GY_ML8511_OUTPUT_VOLTAGE_MIN, GY_ML8511_OUTPUT_VOLTAGE_MAX, 
